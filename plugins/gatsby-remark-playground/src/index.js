...
module.exports = ({ markdownAST }, { customCodeLang = "playground" } = {}) => {
  visit(markdownAST, `code`, node => {
    ...
    if (node.lang === customCodeLang) {
      ...
      node.type = `html`
      node.value = `<pre><code class="language-${customCodeLang}">${escape(node.value)}</code></pre>`;
    }
    if (node.lang === `${customCodeLang}_norender`) {
      node.type = `html`
      node.value = `<pre><code class="language-${customCodeLang}_norender">${escape(node.value)}</code></pre>`;
    }
  });
};